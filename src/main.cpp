...
void scene1(hittable_list& world) {
    // 물체에 사용할 머티리얼
...
// 카메라 테스트 1
void scene2(hittable_list& world) {
    auto R = std::cos(pi / 4);
...
// 삼각형 테스트
void scene3(hittable_list& world) {
    auto material_center = make_shared<lambertian>(color(0.1, 0.2, 0.5));
...
// 폴리곤 메시 테스트
void scene4(hittable_list& world) {
    auto material_center = make_shared<lambertian>(color(0.1, 0.2, 0.5));
    auto material_left = make_shared<metal>(color(0.3, 0.6, 0.8), 1.0);
    auto material_right = make_shared<metal>(color(0.8, 0.6, 0.2), 0.1);
...
void scene5(hittable_list& world) {
    auto material_center = make_shared<lambertian>(color(0.1, 0.2, 0.5));
